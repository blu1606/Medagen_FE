...
interface StepProps {
    ...
    setBodyMapSide?: (side: 'front' | 'back') => void;
    isMounted?: boolean;
}

const StepTriage = ({ t, data, updateData, nextStep, handleEmergency }: StepProps) => (
...
const StepComplaint = ({ t, data, updateData, nextStep, prevStep }: StepProps) => {
    const imageUpload = useImageUpload();
    const isComposing = useRef(false);

    const handleComplaintKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter' && !isComposing.current && data.chiefComplaint) {
            nextStep();
        }
    };
...
const StepDetails = ({ t, data, updateData, prevStep, handleComplete, isLoading, error, bodyMapSide, setBodyMapSide, isMounted }: StepProps) => (
